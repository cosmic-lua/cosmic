 ...
 * ---@field unveil Unveil what alone the child has of the filesystem, or nil for all of it: a root of its own, in namespaces of its own, and System V IPC of its own, holding those paths at the names they resolve to, each given through a link a link there too -- and, with /proc among them and no /dev given whole, /dev/fd and /dev/stdin, /dev/stdout and /dev/stderr as links into it -- and nothing else, so a path outside them is not there to stat any more than to open. A ruleset with it names paths as the child sees them: a rule on a directory above an unveiled path does not reach into it, since each is a mount of its own, so name the unveiled paths themselves. Linux, where unprivileged user namespaces are allowed; ENOSYS elsewhere, and EPERM or the like where they are not
