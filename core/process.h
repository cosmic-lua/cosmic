 ...
 * ---@field unveil Unveil what alone the child has of the filesystem, or nil for all of it: a root of its own, in namespaces of its own, and System V IPC of its own, holding those paths at the names they resolve to, and, for each given through a link, that link there too -- and, with /proc among them and no /dev given whole, /dev/fd and /dev/stdin, /dev/stdout and /dev/stderr as links into it, and, unless /tmp or / is among them, a /tmp of its own that its own children share, empty but for the paths given beneath the host's -- and nothing else, so a path outside them is not there to stat any more than to open. A ruleset with it names paths as the child sees them: a rule on a directory above an unveiled path does not reach into it, since each is a mount of its own, so name the unveiled paths themselves. /proc given to read is read-only but for the processes' own entries (/proc/<pid> and the links into them, /proc/self among them), which stay writable, so the child can in turn start one confined -- which writes its own /proc/self/uid_map -- given a subset of what it has: every other entry that is a directory or a file anyone may write (/proc/sys, /proc/sysrq-trigger and the like) is read-only, and so is every mount beneath it. That lets the child write what its user may of a process of the same user outside it -- its oom_score_adj, its coredump_filter -- as it can signal one; its memory and environment it may not, being in a user namespace of its own. A child root runs, having given up the CAP_SETFCAP that mapping root into a user namespace takes, starts its own confined unmapped: as the kernel's overflow id (65534) inside, with root's access to every file all the same, on a root and a /tmp built in a directory of its TMPDIR, which is left there -- and that one, unmapped, cannot confine one of its own again. Linux, where unprivileged user namespaces are allowed; ENOSYS elsewhere, and EPERM or the like where they are not
