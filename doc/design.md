...
- [x] *a stat's times and inode, declared*: a test whose verdict turns on
  ...
  times, inode, device, link count or owner -- declares it with
  `observations.reads_stat_times()` (build/filesystem_observations.tl), at its
  module's top level or in the test. the capture notes the declaration among
  the test's reads, so the key a stored run is looked up under sees it before
  the test is judged, and every stat and lstat it made of a path of the tree,
  and the stat of every file of the tree it opened, is keyed whole in both its
  own key and its shared key, `o/` included. other checkouts never share an
  inode or a change time with this one, so a declared test's verdict
  effectively never stands in a sibling: it runs there itself, as it must.
  every other test's shared key keeps kind, size and mode; its own key keeps
  every field but the access time, which reading moves, and under `o/` only
  kind, size and mode. no test in the tree turns on times: its stats of the
  tree ask whether a path is there and what it is, and its comparisons of
  inodes (`Fs.walk`'s cycles, `build/refresh.tl`'s `same_file`) ask whether
  two paths are one file, which the tree's links, keyed by lstat and readlink,
  decide.
- [ ] *a stat's times and inode, undeclared* (`build/test.tl`, above
  ...
  stat, or an fstat of a descriptor it opened -- stands on a sibling's
  verdict where they differ. infer the declaration from the fields of a stat's
  answer the test reads.
