...
if [ "${1-}" = zig-cache ]; then
  ...
  # the tree's location (build/filesystem_observations.tl:828), a
  ...
  # that crosses a link out (build/test.tl:457), and the binary's
  # zoneinfo and ca_roots tables (build/test.tl:1914).
