...
if [ "${1-}" = zig-cache ]; then
  ...
  # the tree's location (build/filesystem_observations.tl:959), a
  # stat's times and inode where a test turns on them
  # (build/test.tl:519), a database a connection opened before the
  # test reads (build/filesystem_observations.tl:596) or one attached
  # through the store (core/store.c:319), a call a module took before
  # the capture (build/filesystem_observations.tl:610), a read
  # resolved as it is made (build/filesystem_observations.tl:618 and
  # :685) and an in-tree path that crosses a link out
  # (build/test.tl:461).
