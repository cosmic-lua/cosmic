...
backtick path or a `cosmic.<mod>.<sym>` span in `docs/`, `skills/`,
`README.md` or `AGENTS.md` must still name a real file or a symbol the
doc index carries (`_build/doc_paths_test.tl`, `_build/doc_symbols_test.tl`).
