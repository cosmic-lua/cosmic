...
from the pinned cosmos/tl, with drift tests failing on divergence.
**Done, and the verb it was heading for dissolved with it**:
`_types/types_gen.tl` is a generation unit, `--make build` runs it into
`o/_types/`, and a pin bump is `fetch` then `build` — there is nothing
left for a `regen` verb to do that `build` does not already do.

The `.d.tl` are no longer committed, which is the design's rule applied
to itself rather than excepted from. The cost is stated where it is
paid ([choices.md](choices.md)): a fresh clone cannot resolve `cosmo.*`
...
verbs~~ (fetch → build → test), and docs.yml publishes from a
`--make`-produced `o/docs`. **Done.**
