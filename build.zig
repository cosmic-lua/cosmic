...
pub fn build(b: *std.Build) void {
    ...
    for (targets) |t| {
        const resolved = b.resolveTargetQuery(t.query);
        const exe = core(b, t, release_configuration, resolved, lua, sqlite, miniz, mbedtls, cares, curl, cacert_data, false);
        ...
        const hooked = core(b, t, release_configuration, resolved, lua, sqlite, miniz, mbedtls, cares, curl, cacert_data, true);
    ...
    const checked = core(b, checked_target, sanitized_configuration, baselineHostTarget(b), lua, sqlite, miniz, mbedtls, cares, curl, cacert_data, false);
