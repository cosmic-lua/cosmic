#!/bin/sh
# Builds and exercises real new-format Cosmic runtime artifacts, using the
# real Cosmic runtime rather than the native launcher-contract payload.
set -eu
...
case "$verb" in
  build|test) shift ;;
  *)
    echo "usage: runtime.sh {build|test} ARGS..." >&2
    echo "  runtime.sh build OUTPUT_DIRECTORY [PREBUILT_PREFIX [WRITER PORTABLE_DATABASE]]" >&2
    echo "  runtime.sh test RUNTIME_FIXTURE_DIRECTORY" >&2
    exit 2
    ;;
esac
...
if [ "$verb" = build ]; then
  if [ "$#" -ne 1 ] && [ "$#" -ne 2 ] && [ "$#" -ne 4 ]; then
    echo 'usage: runtime.sh build OUTPUT_DIRECTORY [PREBUILT_PREFIX [WRITER PORTABLE_DATABASE]]' >&2
    exit 2
  fi
  out=$1
  prebuilt=${2-}
  ...
  "$writer" "$root/test/portable/tool.tl" prepare-databases \
    "$out/old.db" "$out/new.db" "$out/basis.db" "$out/missing.db"

  "$writer" "$root/test/portable/tool.tl" write-runtime-fixture \
  ...
  target_id=$(host_target_field "$targets" 1)
  target=$(host_target_field "$targets" 4)
  ...
  "$writer" "$root/test/portable/tool.tl" prepare-artifacts "$out" "$targets"
...
# verb = test. Exercises retained validation, VFS reads, and private prefix
# reads using the real Cosmic runtime.
fixture=${1:?usage: runtime.sh test RUNTIME_FIXTURE_DIRECTORY}
...
(
  cache=$work/cache-environment
  set -- env "COSMIC_PORTABLE_CACHE=$cache" \
    COSMIC_PORTABLE_ADJACENT_A=a COSMIC_PORTABLE_ADJACENT_B=b
  names="COSMIC_PORTABLE_ADJACENT_A COSMIC_PORTABLE_ADJACENT_B"
  i=0
  while [ "$i" -lt 40 ]; do
    name=$(printf 'COSMIC_PORTABLE_MANY_%02d' "$i")
    set -- "$@" "$name=many"
    names="$names $name"
    i=$((i + 1))
  done
  long=COSMIC_PORTABLE_$(printf '%080d' 0)
  set -- "$@" "$long=long" "$fixture/runtime.old" \
    test/portable/tool.tl environment-clean "$cache" $names "$long"
  "$@" > "$work/environment.out"
)
grep -F 'portable environment: clean' "$work/environment.out" >/dev/null
...
COSMIC_PORTABLE_CACHE="$work/cache-release" \
  "$fixture/runtime.release" "$root/test/portable/tool.tl" corrupt-ranges \
...
while read -r corrupt_program corrupt_kind; do
  ...
  (
    ...
    COSMIC_PORTABLE_CACHE="$work/cache-release" "$corrupt_program" \
      test/portable/tool.tl retained-prefix-probe "$release_hash" corrupt
  ...
  if [ "$hook" = after_validation ]; then
    (
      ...
      COSMIC_PORTABLE_CACHE="$cache" \
        ...
        "$program" test/portable/tool.tl retained-prefix-probe \
...
(
  ...
  COSMIC_PORTABLE_CACHE="$work/cache-replace" "$program" \
    test/portable/tool.tl retained-prefix-probe "$new_hash" new > "$work/new.out"
)
...
COSMIC_PORTABLE_CACHE="$work/runtime-timing-driver-cache" \
  "$fixture/runtime.release" "$root/test/portable/tool.tl" time-runtime \
